...
public void saveToken(String token, String refreshToken, Long userId) {
	...
	JwtEntity jwtEntity =
		tokenRepo.findByUserId(userId).orElseGet(() -> createNewJwtEntity(userId));
...
public LogoutResponse kill(long tokenId) {
	...
	if (x == null || x == 0) throw new IllegalArgumentException("Error has occurred");
...
public AuthResponse getUserNameFromTokenUsingRefreshToken(String refreshToken) {
	...
	if (jwtEntity.getValidId() != 1) throw new BusinessException("invalid Token");
	...
	return AuthResponse.builder().accessToken(token).refreshToken(refToken).build();
}
